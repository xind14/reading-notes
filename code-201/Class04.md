...
- Forking Directions: 
  - Fork original > git clone down > make changes locally > ACP, changes end up in fork - new repo so forked changes will not appear in original repo - forked does pull request,base repo original, base main, forked head, compare main - Add index and notes, please merge, create pull request - original sides received it, files have changed, review, merge pull request - original needs to pull down into local - PR goes from children to parent only - forked can name repo different from original can still merge - original owner cannot push changes or else pull request from forked user will create conflicts

- Function: give computer a set of tasks and repeat it the same way every single time.
- Variable `let` only works in that function and not anywhere else, best practice put into function 
- Function Syntax:
